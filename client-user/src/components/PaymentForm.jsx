...
const PaymentForm = () => {
    ...
    const paymentHandler = useCallback(async () => {
        try {
            // get the price first 
            const res = await axios.get(`https://course-server-tquv.onrender.com/users/courses/${id}`, {
                headers: {
            ...
            try {
                const res2 = await axios
                    .post("https://course-server-tquv.onrender.com/payment/create-payment-intent", {
                        course: { price: p }
                    }, {
